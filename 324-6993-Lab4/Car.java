...
public class Car extends Thread {
  ...
  public void run () {
    for (int I=1;  I<= 4; I++) {
      ...
      //get Mutex on traffic light and the counter for the WestEntrance 
      Synch.Mutex.acquire();
      ...
      //If the traffic light is red, OR, if there are any cars in the west bound 
      if ((TrafficLightsController.WestBoundLightRed==true) || (Synch.WestboundCarCounter>0)) 
      {
    	  System.out.println("At time " + Synch.timeSim.curTime() + " Car " + myName + " waits at Westbound red light.\n");

    	  //Increment counter because there are cars at the red light
    	  Synch.WestboundCarCounter++;  

    	  //Release traffic light and Westbound counter mutex 
    	  Synch.Mutex.release();

    	  //The light for Westbound entrance is red, cars should stop and wait
    	  Synch.WestEntrance.acquire(); 
    	  
      }
      else 
      {
    	 //Traffic light is green so release the lock
    	  Synch.Mutex.release();
      ...
      //get Mutex on traffic light and the counter for the eastentrance 
      Synch.Mutex.acquire();

      //If the traffic light is red, OR, if there are any cars in the east bound 
      if ((TrafficLightsController.EastBoundLightRed==true) || (Synch.EastBoundCarCounter>0)) {
    	  System.out.println("At time " + Synch.timeSim.curTime() + " Car " + myName + " waits at Eastbound red light.\n");

    	  //Increment counter because there are cars at the red light
    	  Synch.EastBoundCarCounter++;

    	  //Release traffic light mutex
    	  Synch.Mutex.release();
   
    	  //The light for EastBound entrance is red, cars should stop and wait
